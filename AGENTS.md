...
**Toolchain note:** golangci-lint (2.8.0, built with go1.25) **crashes** — not fails — under a system Go 1.26 (`panic: file requires newer Go version go1.26 ... goanalysis ... convertError`). It's a type-checker panic on go1.26-tagged stdlib, unrelated to your code (`go build`/`go test` pass). Run the gate pinned: `GOTOOLCHAIN=go1.25.8 make ci`. The pin still surfaces real lint findings — it only avoids the panic.
...
**Deploy absorbs gh-aw CLI quirks**: `internal/fleet/deploy.go` works around three upstream `gh aw` behaviors so a `deploy` is reliable across CLI versions — (1) `ensureInit` recognizes both the new (`agentic-workflows.agent.md`) and legacy (`agentic-workflows.md`) init markers, so an already-initialized repo isn't spuriously re-`gh aw init`'d (newer CLIs fail init on repos predating the dispatcher-skill layout); (2) `fixMisplacedSkillImports` relocates a mis-nested `.github/workflows/.github/` skill tree up to `.github/` (a `gh aw add` path bug that drops skill imports one dir too deep) then recompiles the added workflows; (3) `recompileAddedWorkflows` **must forward `--engine`** — `gh aw add --engine X` overrides the engine only for *its own* compile and never rewrites the workflow `.md`, so any *later* `gh aw compile` reverts to the markdown's native engine (e.g. `engine: claude`) and the deployed lock then demands the wrong secret (`ANTHROPIC_API_KEY`) at runtime. **Keep the local `gh aw` CLI version matched to the fleet's source pins** (`fleet.json` refs) — a mismatch breaks `init`/`add`/`compile` in version-specific ways (the CLI both generates and `--strict`-validates lock files, so a skew can make it reject its own output).
...
- Go 1.25.8 (per `go.mod`). + `github.com/spf13/cobra` v1.10.2 (CLI), `github.com/rs/zerolog` v1.35.1 (stderr structured logging), `archive/zip` + `encoding/json` (stdlib, in-memory artifact unzip + decode), `regexp` (stdlib, body-marker parsing). **No new third-party dependencies** (Constitution Principle I). (009-consumption-subcommand)
