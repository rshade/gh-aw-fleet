...
go run . overview     # Join drift, run health, no-op, and AIC/cost across the fleet (read-only).
...
**Overview dashboard**: `gh-aw-fleet overview` is a read-only drift×health×cost join. It builds rows from the scoped `cfg.Repos`, runs `Status()` for drift and the shared `collectRepoRuns` logs fan-out concurrently, then renders `REPO DRIFT RUNS FAIL NOOP HEALTH AIC COST` plus a pooled `TOTAL`. Its default window is `--trailing 7d` (unlike `consumption`, whose default is `--latest`); the same `--latest` / `--trailing Nd` / `--since YYYY-MM-DD` selectors are mutually exclusive. Exit code is drift-only: drifted or errored repos return 1, run failures remain advisory. `NOOP` sums `mcp_tool_usage.summary` entries for `safeoutputs/noop` and clamps the value to the success count; in `--latest` mode that aggregate is best-effort because it cannot be re-filtered per run. JSON output is additive under `cmd.SchemaVersion == 1`.
...
- Go 1.26.4 local toolchain. + `github.com/tailscale/hujson` (BSD-3-Clause, zero transitive deps) retained for the `Add` AST append path and Renovate scanner. Approved direct dependency under [Constitution §Third-Party Dependencies](./.specify/memory/constitution.md). (issue #73 / retained after 016)
- N/A — `internal/fleet/load.go` now reads and patches via `github.com/rshade/ax-go/config`; hujson direct AST mutation (`hujson.Parse`/`Pack`) remains in `Add`, and Renovate config parsing remains in `security/renovate.go`. Probes `<base>.hujson` then `<base>.json`; both present is rejected as ambiguous. (issue #73 / 016)
...
- 013-dependabot-conflict-scanner: a fifth read-only advisory scanner in the slice-006 security registry (`internal/fleet/security/dependabot.go`, wired in `defaultScanners()`) — warns at `LOW` per `github-actions` update entry that does not ignore the gh-aw-actions family. The one structural divergence from the Renovate sibling: Dependabot has **no** file-glob ignore, so there is one conflict rule, not two, and the remedy educates the operator that the generated lock files stay reachable by name (FR-004). Unparseable YAML → one `INFO`; new `security_dependabot` diag code (one family code, per-rule granularity on `Fields["rule_id"]`); reuses `gopkg.in/yaml.v3`, no new deps, no schema-version bump; surfaces on `deploy`/`sync`/`upgrade`.
