...
## Recent Changes
- 014-forecast-subcommand: new read-only `gh-aw-fleet forecast` subcommand fans out `gh aw forecast --json` across the fleet, aggregates per-workflow projections into `ForecastGroup` rows keyed on four axes (`repo|profile|cost-center|tier`), and renders both a text table and a JSON envelope; requires gh-aw ≥ v0.79.2; `--period week|month` selects the projection horizon; no new direct dependencies.
